...
class CGoldenSquare {
public:
    void goldenSquare(double lungime, int nivel, CPunct& p, CVector v) {
        CPunct p1 = p;
        double x, y;
        p.getxy(x, y);
        if (nivel == 0) {
            p1 = CPunct(x + lungime / 3, y - lungime / 3);
            for (int i = 0; i < 4; ++i) {
            ...
            CPunct originalPosition = p;
            goldenSquare(lungime, nivel - 1, p, v);     //centru

            p1 = CPunct(x, y);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //1

            p1 = CPunct(x + lungime / 3, y);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //2

            p1 = CPunct(x + 2 * lungime / 3, y);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //3

            p1 = CPunct(x, y - lungime / 3);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //4

            p1 = CPunct(x + 2 * lungime / 3, y - lungime / 3);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //5

            p1 = CPunct(x, y - 2 * lungime / 3);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //6

            p1 = CPunct(x + lungime / 3, y - 2 * lungime / 3);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //7

            p1 = CPunct(x + 2 * lungime / 3, y - 2 * lungime / 3);
            goldenSquare(lungime / 3, nivel - 1, p1, v);     //8
        }
    }
    ...
    void afisare(double lungime, int nivel) {
        CVector v(1.0, 0.0);
        CPunct p(-1, 1);
        goldenSquare(lungime, nivel, p, v);
    }
};
...
{
public:
    void arboreiregul(double lungime,
        int nivel,
        CPunct p,
        CVector v)
    {
        ...
        else
        {
            ...
            v.deseneaza(p, lungime);
            p1 = v.getDest(p, lungime);
            arboreiregul(lungime * 0.4, nivel - 1, p1, v);
    ...
    void afisare(double lungime, int nivel)
    {
        ...
        v.deseneaza(p, 0.1);
        p = v.getDest(p, 0.1);
        arboreiregul(lungime, nivel, p, v);
    }
};

class CArrowhead {
public:
    void arrowhead(double lungime, int nivel, CPunct & p, CVector & v, int d)
    {
        if (nivel == 0)
        {
            v.deseneaza(p, lungime);
        ...
        else
        {
            ...
            arrowhead(lungime/2, nivel - 1, p1, v1, d);
            arrowhead(lungime / 2, nivel - 1, p2, v2, d);
            arrowhead(lungime/2, nivel - 1, p3, v3, d);
        }
    }
...
// displays the Arbore iregular
void Display9() {
    CArboreIregul cal;
    cal.afisare(0.4, nivel);


    glRasterPos2d(-1.0, -0.9);
    const char* text = "Arbore Iregular";
...
void Display10() {
    ...
    glRasterPos2d(-1.0, -0.9);
    const char* text = "Arrowhead  Sierpinski";
